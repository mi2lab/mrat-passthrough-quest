...
#pragma warning restore CA2213 // Disposable fields should be disposed
        ...
        private bool _CanCleanupCancellationToken => !_loadMeshAsyncSliceHandle.IsValid && !_loadMaterialAsyncSliceHandle.IsValid;
        ...
        private bool AreAllTasksCancelled()
        {
            ...
            if (_texturesDataTask != null)
            {
                ...
                {
                    OvrAvatarLog.LogError($"Cancelled Task {_texturesDataTask} is still running",
                            primitiveLogScope);
                    allCancelled = false;
                }
            }
        ...
        private void LoadAndApplyExtensions()
        {
            var result = CAPI.ovrAvatar2Primitive_GetNumMaterialExtensions(assetId, out uint numExtensions);
            if (!result.EnsureSuccess($"GetNumMaterialExtensions {result}", primitiveLogScope) || numExtensions == 0) {
                return;
            }
            ...
            for (uint extensionIdx = 0; extensionIdx < numExtensions; extensionIdx++)
            {
                if (OvrAvatarMaterialExtension.LoadExtension(
                        assetId,
                        extensionIdx,
                        out OvrAvatarMaterialExtension extension))
                {
                    extension.ApplyEntriesToMaterial(material, _shaderConfig.ExtensionConfiguration);
                }
                else
                {
                    OvrAvatarLog.LogError($"Unable to load material extension at index {extensionIdx}", primitiveLogScope);
                }
            }
        }
